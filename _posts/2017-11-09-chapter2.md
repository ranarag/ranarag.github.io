...
& \text{Initialize, for a } = 1 \text{ to k}: \\
  \begin{align}
  & \displaystyle Q(a) \leftarrow 0 \\
  & \displaystyle N(a) \leftarrow 0 
  \end{align} \\
& \displaystyle \text{Repeat Forever:} \\
...
& \displaystyle R \leftarrow bandit(a) \\
& \displaystyle \displaystyle N(A) \leftarrow N(A) + 1 \\
& \displaystyle Q(A) \leftarrow Q(A) + \frac{1}{N(A)}[R - Q(A)]
\end{align}$$


The  $$\frac{1}{N(A)} parameter (a.k.a *StepSize*) changes from time step to time step and can also be written as $$\alpha _{t}(a)$$.
